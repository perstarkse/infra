{
  config.flake.nixosModules.router-core = {
    ...
    inherit (lib) all attrNames concatMap filter hasAttr listToAttrs mapAttrs mapAttrsToList mkEnableOption mkOption nameValuePair optional optionals types unique;

    ipv4OctetPattern = "(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]?|0)";
    ...
    segmentSubmodule = types.submodule (_: {
      options = {
        ...
        policy = {
          ...
          routerAllowedTcpPorts = mkOption {
            type = types.listOf types.int;
            default = [];
            description = "Additional router TCP ports reachable from this segment in addition to the ports granted by routerAccessLevel.";
          };
    ...
    routerServicePortSubmodule = types.submodule {
      options = {
        access = mkOption {
          type = types.enum ["infra" "admin" "wan"];
          description = "Router service access class";
        };
        protocol = mkOption {
  ...
  in {
    options = {
      my.router = {
            ...
            type = types.listOf (types.submodule {
              options = {
                ...
                cloudflareProxied = mkOption {
                  type = types.bool;
                  default = false;
                  description = "Require requests to arrive through Cloudflare edge IPs (uses updatable snippet). LAN/WireGuard access remains allowed.";
                };
                noAcme = lib.mkOption {
                  type = lib.types.bool;
                  default = false;
        ...
        casting = {
          enable = mkEnableOption "Enable trusted-to-media casting helpers";
          sourceSegment = mkOption {
            type = types.str;
            default = cfg.primarySegment;
            description = "Segment from which user devices initiate casting.";
          };
          targetSegments = mkOption {
            type = types.listOf types.str;
            default = [];
            description = "Segments containing cast targets like TVs and streamers.";
          };
          mdns.reflector = mkOption {
            type = types.bool;
            default = true;
            description = "Enable Avahi mDNS reflection across the casting interfaces.";
          };
          allowChromecast = mkOption {
            type = types.bool;
            default = true;
            description = "Allow common Chromecast / Google TV control ports from source to target segments.";
          };
          extraTcpPorts = mkOption {
            type = types.listOf types.int;
            default = [];
            description = "Additional TCP ports to allow from casting source to targets.";
          };
          extraUdpPorts = mkOption {
            type = types.listOf types.int;
            default = [1900 5353];
            description = "Additional UDP ports to allow from casting source to targets.";
          };
        };
      ...
      my.router.internalServicePorts = mkOption {
        type = types.listOf routerServicePortSubmodule;
        default = [];
        internal = true;
        description = "Router-local service ports exported by router submodules";
      };
    };
    ...
    config = lib.mkIf cfg.enable {
        ...
        allReservationIps =
          concatMap (
            name: let
              ...
              machineReservations = map (m: "${segment.subnet}.${m.ip}") (filter (machine:
                (
              ...
              cfg.machines);
            in
              directReservations ++ machineReservations
          )
          segmentNames;
        allReservationMacs =
      ...
      in [
        ...
        {
          assertion = all (m: isMacAddress m.mac) cfg.machines;
          message = "my.router.machines.*.mac must be a MAC address like 00:11:22:33:44:55";
        }
        {
        ...
        }
        {
          assertion =
            all (
              name: all (r: isMacAddress r.mac) cfg.segments.${name}.dhcp.reservations
            )
            segmentNames;
          message = "my.router.segments.*.dhcp.reservations.*.mac must be a MAC address like 00:11:22:33:44:55";
        }
        {
          assertion =
            all (
        ...
        }
        {
          assertion =
            all (
              name: let
                inherit (cfg.segments.${name}) policy;
              in
        ...
        {
          assertion = all (entry: entry.port >= 1 && entry.port <= 65535) config.my.router.internalServicePorts;
          message = "my.router.internalServicePorts.*.port must only contain ports in range 1..65535";
        }
        {
          assertion = !cfg.casting.enable || lib.hasAttr cfg.casting.sourceSegment cfg.segments;
          message = "my.router.casting.sourceSegment must reference a declared segment";
        }
        {
          assertion = all (name: lib.hasAttr name cfg.segments) cfg.casting.targetSegments;
          message = "my.router.casting.targetSegments must only reference declared segments";
        }
        {
          assertion = all (port: port >= 1 && port <= 65535) (cfg.casting.extraTcpPorts ++ cfg.casting.extraUdpPorts);
          message = "my.router.casting.extra{Tcp,Udp}Ports must only contain ports in range 1..65535";
        }
      ];
      ...
      routerHelpers = let
        ...
        in {
          ...
          dhcp = {
            ...
            domainName =
              if segment.dhcp.domainName != null
              then segment.dhcp.domainName
              else cfg.dhcp.domainName;
