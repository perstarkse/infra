{
  ...
  in {
    ...
    config = mkMerge [
      ...
      {
        my.secrets.declarations = lib.flatten (mapAttrsToList (
            name: backup:
              lib.optional backup.enable (
                config.my.secrets.mkMachineSecret {
                  name = "restic-${name}";
                  dependencies =
                    if backup.backend.type == "b2"
                    then ["b2-service"]
                    else if backup.backend.type == "s3"
                  ...
                  script = let
                    ...
                    mkRepoUrl =
                      if backup.backend.type == "b2"
                      then "b2:${
                        if backup.backend.bucket != null
                        then backup.backend.bucket
                        else mkBucketName
                      }:${name}"
                      else if backup.backend.type == "s3"
                      then "s3:${
              ...
              lib.nameValuePair "restic-bootstrap-${name}" (mkIf (backup.enable && !backup.restore.enable) {
                ...
                script = let
                  bucketName =
                    if backup.backend.bucket != null
                    then backup.backend.bucket
                    else "restic-${config.networking.hostName}-${name}";
                in ''
                  set -euo pipefail
