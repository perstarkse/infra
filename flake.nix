{
  ...
  outputs = {
    ...
    declaredInputs = import ./flake/inputs.nix;
    missingInputs = builtins.filter (name: !(inputs ? ${name})) (builtins.attrNames declaredInputs);
  in
    assert missingInputs == [];
      flake-parts.lib.mkFlake {inherit inputs;} ({config, ...}: {
        ...
        perSystem = {
          ...
          mainTopology = import ./flake/main-topology.nix {
            inherit lib;
            inherit pkgs;
            inherit (inputs.self.topology.${system}.config) nodes;
          };
          networkTopology = import ./flake/network-topology.nix {
            inherit lib;
            inherit pkgs;
            inherit (inputs.self.topology.${system}.config) nodes;
            inherit (inputs.self.topology.${system}.config) networks;
          };
          servicesTopology = import ./flake/services-topology.nix {
            inherit lib;
            inherit pkgs;
            inherit (inputs.self.topology.${system}.config) nodes;
          };
