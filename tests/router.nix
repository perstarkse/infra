...
in {
  ...
  router-dns-enforcement = pkgs.testers.runNixOSTest {
    ...
    testScript = ''
      ...
      router.succeed("nft list table ip natV4 | grep -Eq 'iifname \"vlan20\".*dport 53.*redirect'")
      router.succeed("nft list table ip filterV4 | grep -Eq 'iifname \"vlan20\".*oifname \"eth1\".*tcp dport 853.*reject'")
      # LAN is IPv4-only: the ip6 tables must not carry any segment rules.
      router.fail("nft list table ip6 filterV6 | grep -Eq 'vlan20'")
      router.fail("nft list table ip6 natV6")
    '';
  };
